...
async function fetchWeeklyInsights(igUserId, token, sinceUnix, untilUnix) {
  // A Graph API descontinuou a métrica "impressions" no nível de conta —
  // "views" é a substituta atual (mesmo conceito, nome novo). "views" e
  // "profile_views" agora exigem metric_type=total_value, que resume o
  // período todo num único total_value.value em vez de valores por dia.
  ...
  const sums = { reach: 0, views: 0, profile_views: 0 };
  (json.data || []).forEach(metric => {
    sums[metric.name] = (metric.total_value && metric.total_value.value) || 0;
  });
  return { reach: sums.reach, impressions: sums.views, profileVisits: sums.profile_views };
}

// Quantas contas começaram e quantas deixaram de seguir no período. A
// métrica "follows_and_unfollows" com breakdown "follow_type" não é bem
// documentada pela Meta, mas foi confirmada empiricamente contra a API real
// desta conta: o valor "FOLLOWER" bate exatamente com a soma diária de
// follower_count (= ganhos), então "NON_FOLLOWER" é a contagem de perdas.
async function fetchFollowsAndUnfollows(igUserId, token, sinceUnix, untilUnix) {
  const json = await graphGet(`/${igUserId}/insights`, {
  ...
  const results = (json.data && json.data[0] && json.data[0].total_value
    && json.data[0].total_value.breakdowns && json.data[0].total_value.breakdowns[0]
    && json.data[0].total_value.breakdowns[0].results) || [];
  const byType = {};
  results.forEach(r => { byType[r.dimension_values[0]] = r.value; });
  return { gained: byType.FOLLOWER || 0, lost: byType.NON_FOLLOWER || 0 };
}

async function fetchMediaSince(igUserId, token, sinceUnix, untilUnix) {
  const json = await graphGet(`/${igUserId}/media`, {
    fields: 'id,caption,permalink,timestamp,like_count,comments_count,media_type,media_product_type,thumbnail_url,media_url',
    since: sinceUnix,
    until: untilUnix,
    access_token: token
  });
...
// "media_type" (IMAGE/VIDEO/CAROUSEL_ALBUM) + "media_product_type" (distingue
// Reels de vídeo de feed comum) viram um único rótulo pra exibição/agrupamento.
function mediaFormatLabel(mediaType, mediaProductType) {
  if (mediaType === 'CAROUSEL_ALBUM') return 'Carrossel';
  if (mediaType === 'IMAGE') return 'Imagem';
  if (mediaType === 'VIDEO') return mediaProductType === 'REELS' ? 'Reels' : 'Vídeo';
  return 'Outro';
}
...
async function fetchStoriesSince(igUserId, token, sinceUnix, untilUnix) {
  const json = await graphGet(`/${igUserId}/stories`, {
    // "exits" saiu da lista de métricas válidas de story da Graph API — sem substituto direto.
    fields: 'id,timestamp,insights.metric(impressions,reach,replies)',
    since: sinceUnix,
    until: untilUnix,
    access_token: token
  });
