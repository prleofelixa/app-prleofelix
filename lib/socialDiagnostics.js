...
function diagnose({ current, previous, posts = [], contentItemsThisWeek = [], contentItemsPrevWeek = [], contentById = {} }) {
  ...
  if (!previous) {
    ...
    if (growthPct === null) {
      ...
      push('crescimento', '📈', 'Crescimento', 'warn',
        `perda de seguidores esta semana (${fmtPct(growthPct)}, de ${fmtNum(igPrev.followers)} para ${fmtNum(ig.followers)}).`);
      ...
      push('crescimento', '📈', 'Crescimento', 'warn',
        `desacelerou em relação à semana passada — ${fmtPct(growthPct)} nesta semana contra ${fmtPct(prevGrowthPct)} na anterior (${fmtNum(ig.followers)} seguidores).`);
      ...
      push('crescimento', '📈', 'Crescimento', 'good',
        `acelerou — ${fmtPct(growthPct)} nesta semana contra ${fmtPct(prevGrowthPct)} na anterior (${fmtNum(ig.followers)} seguidores). Vale identificar o que impulsionou e repetir a fórmula.`);
    } else {
      push('crescimento', '📈', 'Crescimento', 'neutral', `estável, ${fmtPct(growthPct)} nesta semana (${fmtNum(ig.followers)} seguidores).`);
    }
  }
  ...
  push('youtube', '▶️', 'YouTube', ytTone,
    `${fmtNum(yt.subscribers)} inscritos (${yt.subscribersGained >= 0 ? '+' : ''}${fmtNum(yt.subscribersGained)} na semana), ${fmtNum(yt.views)} visualizações, ${fmtNum(yt.watchTimeHours)}h de tempo de exibição.`);
